...
def matrixtoposubmerge(img, matrix_img, rise, count):
    ...
    for pixel in submerge:
        x,y = pixel
        img_colorpixels[x,y] = (0,0,255)
    save(img_colors, (img_name + '_' + str(count)))

def matrixsubmergemain():
    img = open(input('File name: '))
    precision = int(input('Precision: '))
    print('Creating matrix...')
    startmatrix = time.time()
    matrix_img = matrix_create(img, precision)
    endmatrix = time.time()
    ...
    while cont:
        ...
        for i in range(int(riseupto/riseinc) + 1):
            start = time.time()
            matrixtoposubmerge(img, matrix_img, round(i*riseinc,5), count)
            end = time.time()
            count += 1
...
def savematrix():
    ...
    print(str(matrix_img))
    textfile = input('Text file name: ')
    with open(textfile, 'w') as w:
        w.write(str(matrix_img))
    print('Saved matrix in', textfile)
